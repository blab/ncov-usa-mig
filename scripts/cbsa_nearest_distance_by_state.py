...
for s1_full in state_full_list:
    ...
    for s2_full in state_full_list:
        ...
        if s1_full == s2_full:
            records.append(
                {"x": s1_full, "y": s2_full, "min_cbsa_distance_km": 0}
            )
        ...
        if math.isfinite(best):
            records.append(
                {"x": s1_full, "y": s2_full, "min_cbsa_distance_km": round(best, 2)}
            )
pd.DataFrame(records).to_csv("data/state_pair_min_cbsa_distance_km.csv",index=False)
